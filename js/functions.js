(function($){
   "use strict";	
	...
	$(window).load(function() {	
		...
		//------------> Scroll to anchore when window is loaded with
		if ($(location).attr('href').indexOf('#') !== -1){
			var anc = $(location).attr('href').split("#").pop();
			if (anc != undefined && anc != "" && $('#' + anc).length)
				//$('a.scroll[href="#' + anc + '"]').trigger("click");
				$('html, body').animate({ scrollTop: $('#' + anc).offset().top - $('#navigation_bar').outerHeight() + "px"}, 1200, 'easeInOutExpo');
		}
	});	
$(document).ready(function(){
			...
			$('.popup-with-move-anim').magnificPopup({
				...
				callbacks: {
					open: function() {
						...
						if (this.currItem.src == "#login-popup"){
							login_init();
						}
					}
				}
	...
	var login_init = function() {
		$('.lfc_user_row .loading').addClass('mfp-hide');
		$('.lfc_user_row .success').addClass('mfp-hide');
		$('.lfc_user_row .error').addClass('mfp-hide');
		$('.lfc_user_row .problem').addClass('mfp-hide');
	}
/*	
	...
	*/	
	var login_validator = $("#login_form").validate({
		...
		/*errorPlacement: function(error, element) {
			if ($.inArray(element.attr("name").substr(0, element.attr("name").length - 8), ['tel', 'fax', 'email']) != -1)
			...
			// Update form height
			if (element.closest('.pp_inline').length)
				$('.pp_content').height($('.pp_content').height() + 21);
		}*/
	});

	$(document).on('submit', 'form#login_form', function(e){
		var DOMAIN = "http://wsp.sms-algerie.com";
		login_init();
		if (login_validator.form()){			
			$('.lfc_user_row .loading').removeClass('mfp-hide');
			var login_username = $('#login_user_name').val();
			var login_password = $('#login_password').val();
			
			// Request data using post
			var requesting = $.ajax({url:DOMAIN + "/index/auth", method:'POST', dataType:'json', xhrFields: {withCredentials: true}, data:{Login:login_username, PWCompte:login_password}})
			.done(function( data ){
				if (data.success && data.success == "true"){
					$('.lfc_user_row .loading').addClass('mfp-hide');
					$('.lfc_user_row .success').removeClass('mfp-hide');
					var timeoutID = window.setTimeout(function(){
						document.location.href = DOMAIN + data.msg;
				...
				if (data.failure && data.failure == "true"){
					...
					$('.lfc_user_row .loading').addClass('mfp-hide');
					$('.lfc_user_row .error').removeClass('mfp-hide');
				}				
			})
			.fail(function() {
				// ... CONNECTION ERROR ...
				$('.lfc_user_row .loading').addClass('mfp-hide');
				$('.lfc_user_row .problem').removeClass('mfp-hide');
	...
	*/	
	var contact_init = function() {
		$('.form_row .loading').addClass('mfp-hide');
		$('.form_row .success').addClass('mfp-hide');
		$('.form_row .error').addClass('mfp-hide');
		$('.form_row .problem').addClass('mfp-hide');
	}

	var contact_validator = $("#contact_us_form").validate({
		rules: {
			contact_us_name: {
				required: true
			},
		...
		messages: {
			...
			contact_us_mail: {
				...
				email: "Un email valide doit être sous forme nom@domaine.tld"
			},
			contact_us_message: {
	...
	$(document).on('submit', 'form#contact_us_form', function(e){
		var DOMAIN = "/";
		//var DOMAIN = "";
		contact_init();
		if (contact_validator.form()){			
			$('.form_row .loading').removeClass('mfp-hide');
			var name = $('#contact_us_name').val();
			var email = $('#contact_us_mail').val();
			var subject = $('#contact_us_subject').val();
			var message = $('#contact_us_message').val();
			
			// Request data using post
			var requesting = $.ajax({
					url:DOMAIN+"php/phpmailer/emailfromcontact.php", 
					method:'POST', 
					dataType:'json', 
					data:{
			...
			.done(function( data ){
				if (data.success){
					$('.form_row .loading').addClass('mfp-hide');
					$('.form_row .success .hint').html(data.message);
					$('.form_row .success').removeClass('mfp-hide');
					var timeoutID = window.setTimeout(function(){
						$('.form_row .success').addClass('mfp-hide');
						$('form#contact_us_form').trigger('reset');
						clearInterval(timeoutID);
					}, 8000);
				}
				else{
					// ... AUTHENTICATION ERROR ...
					$('.form_row .loading').addClass('mfp-hide');
					$('.form_row .error .hint').html(data.message);
					$('.form_row .error').removeClass('mfp-hide');
				}				
			})
			.fail(function() {
				// ... CONNECTION ERROR ...
				$('.form_row .loading').addClass('mfp-hide');
				$('.form_row .problem').removeClass('mfp-hide');
	...
	*/	
	...
	$(document).on('submit', 'form#api_form', function(e){
		//var DOMAIN = "https://www.sms-algerie.com/";
		var DOMAIN = "/";
		//var SENDER = "php/phpmailer/emailfromapi.php";
		var SENDER = "messages/email_from_api_via_sendgrid.php";
		api_init();
		if (api_validator.form()){			
			$('.form_row .loading').removeClass('mfp-hide');
			var name = $('#api_us_name').val();
			var email = $('#api_us_mail').val();
			var entreprise = ($('#api_us_entreprise').val()) ? $('#api_us_entreprise').val() : '';
			var hp = ($('#api_us_hp').val()) ? $('#api_us_hp').val() : '';
			
			// Request data using post
			var requesting = $.ajax({url:DOMAIN+SENDER, method:'POST', dataType:'json', data:{api_us_name:name, api_us_mail:email, api_us_entreprise:entreprise, api_us_hp:hp}})
			.done(function( data ){
				if (data.success){
					$('.form_row .loading').addClass('mfp-hide');
					$('.form_row .success .hint').html(data.message);
